...
float rb = 0.35;
float angle= 0;
int[] loopTimeGen= IntStream.rangeClosed(1, 16).toArray();
...
while(1==1) {
    ...
    if(haplyBoard.data_available()){
      ...
      if(start==true){      
        ...
        yr=ra*cos((float)Math.toRadians(angle));
        xr=rb*sin((float)Math.toRadians(angle));
 ...
 // Torques from difference in endeffector and setpoint, set gain, calculate force
    ...
    println("Caution, process loop took: " + timetook/1000000.0 + "ms");
      ...
      if(random == true){
        looptime = loopTimeGen[new Random().nextInt(loopTimeGen.length)]*250;
      }
    }
